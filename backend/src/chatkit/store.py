...
from chatkit.types import (
    ...
    HiddenContextItem,
    ActiveStatus,  # Use ActiveStatus instead of ThreadStatus
    FileAttachment,
    ImageAttachment,
...
class TodoChatKitStore(Store[dict]):
    ...
    async def delete_thread(
        ...
        if conversation and conversation.user_id == user_id:
            ...
            messages = (await session.exec(msg_query)).all()
            for msg in messages:
    ...
    async def add_thread_item(
        ...
        message = self._thread_item_to_message(conv_id, item)
        session.add(message)
        await session.commit()
    ...
    def _thread_item_to_message(self, conversation_id: int, item: ThreadItem) -> Message:
